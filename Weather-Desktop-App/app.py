...
class Weather(Tk):
    ...
    def __gui(self):
       # Load city list for autocomplete
        with open("city_list.json", "r") as file:
            ...
            self.city_names = [city['name'] for city in city_data]
        ...
        # Autocomplete city search box
        self.search = StringVar()
        self.search_textbox = ttk.Combobox(self, textvariable=self.search, font=("Segoe UI", 14, 'bold'), width=23, justify="center")
        self.search_textbox["values"] = self.city_names  # Add city names for autocomplete
       ...
       # bottom bar
        ...
        Label(image=self.finalimg6,bg='#E6E6FA').place(x=5,y=330)
       ...
       # visibility label
        ...
        # Graph Button for Last 5 Days Weather
        self.graph_btn = Button(self, text="Show Graph", font=("Georgia", 16, "bold"), bg='orange', fg='black', width=12, relief='groove', command=self.show_graph)
        self.graph_btn.place(x=370, y=420)
    ...
    def show_graph(self):
        ...
        if not geo_response or 'lat' not in geo_response[0] or 'lon' not in geo_response[0]:
            messagebox.showerror("Error", "City not found! Check the spelling and try again.")
            return
        ...
        if "list" not in response:
            messagebox.showerror("Error", "Failed to fetch forecast data. Try again later.")
            return

        # Extract last 5 days of temperature data
        dates = []
        temps = []

        for entry in response['list']:
            date = datetime.datetime.fromtimestamp(entry['dt'], tz=datetime.UTC).strftime('%Y-%m-%d %H:%M')  # ✅ Fixed
            temp = int(entry['main']['temp'] - 273.15)  # Convert Kelvin to Celsius
            dates.append(date)
            temps.append(temp)
        ...
        # Use a built-in Matplotlib style (alternative to seaborn-darkgrid)
        plt.style.use("ggplot")  # ✅ Fixed

        # Plot Graph
        plt.figure(figsize=(12, 6))
        plt.plot(dates, temps, marker='o', linestyle='-', color='#007BFF', markersize=8, linewidth=3, label='Temperature (°C)')
        plt.fill_between(range(len(dates)), temps, min(temps) - 2, color='#007BFF', alpha=0.2)

        plt.xlabel('Date & Time', fontsize=12, fontweight='bold', color='darkblue')
        plt.ylabel('Temperature (°C)', fontsize=12, fontweight='bold', color='darkred')
        plt.title(f'Temperature Trend in {city} (Next 5 Days)', fontsize=14, fontweight='bold', color='darkgreen')

        plt.xticks(range(len(dates)), dates, rotation=45, fontsize=10, color='black')
        plt.grid(True, linestyle='--', linewidth=0.7, alpha=0.7)

        for i, temp in enumerate(temps):
            plt.text(i, temps[i] + 0.5, f"{temp}°C", ha='center', fontsize=10, fontweight='bold', color='black')
